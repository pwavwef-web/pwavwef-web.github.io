...
export interface ProductionRun {
  ...
  step: 'speech' | 'chain' | 'video' | 'insert' | 'composite' | 'trim' | 'extend_upload';
...
async function loadVersion(p: Prod, versionId: string | null): Promise<ProductionVersionDoc | null> {
...
async function loadVersions(p: Prod): Promise<ProductionVersionDoc[]> {
...
async function submitVideo(p: Prod, over: Partial<VideoJobRequest> & { mode: VideoJobRequest['mode']; prompt: string }, label: string, toShot = true): Promise<string> {
  ...
  const req: VideoJobRequest = {
    ...
    label: `${p.title} · ${label}`.slice(0, 160),
    target: toShot ? { kind: 'shot', id: p.shotId } : { kind: 'production', id: p.id },
    ...(base.title ? { title: base.title } : {}),
  };
  const res = await createJobs(p.ownerUid, [req], { productionId: p.id, preconfirmed: true, batchLabel: req.label });
  return res.jobIds[0]!;
}

async function inspectionReferences(p: Prod): Promise<InspectReference[]> {
  const ids = [...p.expected.characters.map((c) => c.referenceAssetId).filter((x): x is string => Boolean(x)).slice(0, 4), ...(p.expected.storyboardAssetId ? [p.expected.storyboardAssetId] : [])];
  if (!ids.length) return [];
  const snaps = await db.getAll(...ids.map((id) => col.assets().doc(id)));
  return snaps
    .filter((s) => s.exists && s.get('kind') === 'image' && s.get('status') === 'ready')
    .map((s) => {
      const character = p.expected.characters.find((c) => c.referenceAssetId === s.id);
      return { assetId: s.id, storagePath: String(s.get('storagePath')), mimeType: String(s.get('mimeType')), label: character ? `Character reference: ${character.name}` : 'Storyboard frame for this shot' };
    });
}
...
async function submitInspection(p: Prod, v: ProductionVersionDoc, stage: 'inspect' | 'reinspect'): Promise<Prod> {
  ...
  const params: InspectParams = {
    ...
    references,
    previousShot,
    label: `${p.title} · version ${v.index}`,
  };
  const estimate = estimateInspection({ modelId: MODEL_REGISTRY.reasoning.id, durationSec: v.durationSec ?? 8, referenceImages: references.length + (previousShot ? 1 : 0), promptChars: 7000 }, PRICING);
  const jobId = await createInternalJob(p.ownerUid, { type: 'quality.inspect', projectId: p.projectId, modelId: MODEL_REGISTRY.reasoning.id, label: `Quality review · ${p.title} v${v.index}`, params: params as unknown as Record<string, unknown>, estimate, target: { kind: 'production', id: p.id, sub: v.id }, productionId: p.id });
  return setState(p, { status: 'inspecting', stage, waitingOn: [jobId], stageMessage: `Inspecting version ${v.index}: transcribing the dialogue and reviewing picture, performance and continuity` });
}
...
async function createVersion(p: Prod, input: { kind: VersionKind; parentVersionId: string | null; repair: RepairDecision | null; jobIds: string[]; takeId: string | null; assetId: string; interactionId: string | null; interactionAt?: number | null; plannedCuts: number[]; editorialCuts?: EditorialWindow[]; label: string }): Promise<{ p: Prod; v: ProductionVersionDoc }> {
...
// ---------------------------------------------------------------------------
// Stage: plan → audio_prepare → duration_calculate
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Stage: generate (single shot or connected shots joined by Omni extension)
// ---------------------------------------------------------------------------
...
async function startChain(p: Prod, plan: DurationPlan, run: ProductionRun, extra: string): Promise<void> {
  ...
  const jobId = await submitVideo(p, { mode: 'generate', prompt, durationSec: seg.durationSec }, plan.segments.length > 1 ? `part 1 of ${plan.segments.length}` : run.kind === 'repair' ? REPAIR_LABELS[run.type!] : 'generation');
  await setState(p, {
    status: run.kind === 'repair' ? 'repairing' : 'generating',
    stage: run.kind === 'repair' ? 'repair' : 'generate',
    waitingOn: [jobId],
    chain: { segmentIndex: 0, takeIds: [], assetIds: [] },
    run: { ...run, step: 'chain', data: { ...run.data, extra, chainJobIds: [jobId] } },
    stageMessage: plan.segments.length > 1 ? `Generating part 1 of ${plan.segments.length} (${seg.durationSec} s)` : `Generating ${seg.durationSec} s with ${VIDEO_CAPABILITIES.displayName}`,
  });
}
...
async function stepChain(p: Prod, jobs: JobDoc[]): Promise<void> {
  ...
  if (nextIndex < plan.segments.length) {
    ...
    await setState(p, { waitingOn: [jobId], chain: { ...chain, segmentIndex: nextIndex }, run: { ...run, data: { ...run.data, chainJobIds: [...((run.data.chainJobIds as string[]) ?? []), jobId] } }, stageMessage: `Connected shot ${nextIndex + 1} of ${plan.segments.length}: extending the take by ${seg.durationSec} s with continuous dialogue` });
    return;
  }
...
// ---------------------------------------------------------------------------
// Stage: inspect / reinspect → decide
// ---------------------------------------------------------------------------

export function estimateRepairUsd(p: Pick<Prod, 'request' | 'plan'>, v: Pick<ProductionVersionDoc, 'durationSec' | 'chainSec'>, d: RepairDecision): number {
  ...
  const inspect = (secs: number) => estimateInspection({ modelId: MODEL_REGISTRY.reasoning.id, durationSec: secs, referenceImages: 3, promptChars: 7000 }, PRICING).usd;
  const video = (outputSeconds: number, videoInputSeconds: number, task: string, imageInputs = 0) => estimateVideo({ resolution: res, outputSeconds, promptChars: 2500, imageInputs, videoInputSeconds, task }, PRICING).usd;
  ...
  switch (d.type) {
    ...
    default:
      return video(d.durationSec ?? D, 0, 'generate', imgs) + inspect(d.durationSec ?? D);
  }
}
...
async function stepInspected(p: Prod, jobs: JobDoc[]): Promise<void> {
  ...
  const repairs = p.repairs.map((r, i) => (i === p.repairs.length - 1 && r.outcome === 'pending' ? { ...r, outcome: (report.passed ? 'fixed' : 'not_fixed') as typeof r.outcome } : r));
  if (report.passed) {
...
export async function startRepair(p: Prod, v: ProductionVersionDoc, d: RepairDecision, estimateUsd: number, categories: string[], directorRequested: boolean): Promise<void> {
  const record = { versionId: v.id, type: d.type, reason: d.reason, categories, jobIds: [] as string[], estimateUsd, at: Date.now(), outcome: 'pending' as const };
  ...
  switch (d.type) {
    ...
    case 'regenerate_section': {
      ...
      if (start > VIDEO_CAPABILITIES.maxEditInputSeconds || start < CAPS.minSec - 0.5) {
        const alt: RepairDecision = { ...d, type: 'regenerate_longer', reason: `${d.reason} (the section starts too late to regenerate on its own, so the scene is regenerated)`, durationSec: Math.min(CAPS.maxSec, Math.max(CAPS.minSec, Math.ceil(v.durationSec ?? 8))) };
    ...
    case 'split_into_shots': {
      ...
      await ref(p.id).set({ plan, repairCount: common.repairCount, repairs: [...p.repairs, record] }, { merge: true });
      await startChain({ ...p, plan, repairCount: common.repairCount, repairs: [...p.repairs, record] }, plan, { ...run, step: 'chain' }, d.instruction);
      return;
    }
    default: {
      // Regenerate as one shot of the chosen length with the full dialogue and firmer direction.
      const secs = Math.min(CAPS.maxSec, Math.max(CAPS.minSec, d.durationSec ?? Math.ceil(v.durationSec ?? 8)));
      const reference = p.plan ?? planFor(p, secs);
      const seg: PlanSegment = { index: 0, durationSec: secs, startSec: 0, endSec: secs, units: p.expected.lines.map((l) => ({ lineIndex: l.index, character: l.character, text: l.text, seconds: 0 })), speechStartSec: p.expected.lines.length ? reference.breakdown.openingSec : null, speechEndSec: p.expected.lines.length ? Math.max(0, secs - reference.breakdown.closingSec - reference.breakdown.actionTailSec) : null, kind: 'opening', camera: '' };
      const single: DurationPlan = { ...reference, plannedSec: secs, strategy: 'single', segments: [seg] };
      const prompt = segmentPrompt(base, single, seg, `Director's note for this regeneration: ${d.instruction}`);
...
async function stepRepair(p: Prod, jobs: JobDoc[]): Promise<void> {
  ...
  if (run.step === 'insert') {
    ...
    await setState(p, { waitingOn: [jobId], run: { ...run, step: 'composite', data: { ...run.data, insertAssetId, insertJobId: job.id } }, stageMessage: 'Cutting the insert in over the original dialogue' });
    return;
  }
  if (run.step === 'trim') {
    ...
    await setState(p, { waitingOn: [jobId], run: { ...run, step: 'extend_upload', data: { ...run.data, trimmedAssetId } }, stageMessage: 'Regenerating only the failed section from the kept part' });
    return;
  }
  ...
  let cuts: number[] = [];
  if (d.type === 'conversational_edit' || d.type === 'extend_scene') cuts = [...base.plannedCuts];
  ...
  const windows = ['conversational_edit', 'extend_scene', 'trim_ending', 'cutaway', 'replace_visuals_keep_audio'].includes(d.type) ? (base.editorialCuts ?? []).filter((w) => w.startSec < keepUntil - 0.1).map((w) => ({ ...w, endSec: Math.min(w.endSec, keepUntil) })) : [];
  const { p: next, v } = await createVersion(p, { kind: 'repair', parentVersionId: base.id, repair: d, jobIds: [...(run.data.insertJobId ? [String(run.data.insertJobId)] : []), job.id], takeId: out.takeId, assetId: out.assetId, interactionId: out.interactionId, plannedCuts: [...new Set(cuts)].filter((c) => c > 0).sort((a, b) => a - b), editorialCuts: windows, label: REPAIR_LABELS[d.type] });
...
async function onChildFailure(p: Prod, job: JobDoc | undefined, run: ProductionRun): Promise<void> {
  ...
  const fakeJob = { status: 'completed', result: { reportId: report.id } } as JobDoc;
  await stepInspected({ ...p, repairs, currentVersionId: base.id, run: null }, [fakeJob]);
}
...
export async function advanceProduction(productionId: string): Promise<void> {
  ...
  try {
    ...
    const limit = /limit/i.test(err.message) && e instanceof HttpsError && e.code === 'resource-exhausted';
    await setState(p, {
      status: 'failed_review',
      waitingOn: [],
      stageMessage: limit ? `Paused — ${err.message}` : `Production stopped: ${err.message}`,
      failure: { summary: err.message, failed: [err.message], attempted: p.repairs.map((r) => REPAIR_LABELS[r.type]), strongestVersionId: p.bestVersionId, nextAttemptUsd: null, options: limit ? ['Raise the spending limit in Settings, then choose Repair or Regenerate'] : manualOptions(false) },
    });
  } finally {
...
export function reevaluate(report: QualityReportDoc, p: Pick<Prod, 'settings' | 'expected'>, waived: string[], plannedCuts: number[], editorialCuts: EditorialWindow[] = []) {
  return evaluateQuality({ dialogue: report.dialogue, review: report.review, measurements: report.measurements, settings: p.settings, plannedCuts, editorialCuts, hasCharacters: p.expected.characters.length > 0, waivedCategories: waived });
}

/** Initial estimate for a production: guide audio + planned generation(s) + one inspection (a review: the inspection only). */
export function productionEstimate(p: Pick<Prod, 'dialogueAudio' | 'request' | 'plan'>, plan: DurationPlan, opts: { reviewSec?: number } = {}) {
  if (opts.reviewSec !== undefined) return sumEstimates([estimateInspection({ modelId: MODEL_REGISTRY.reasoning.id, durationSec: opts.reviewSec, referenceImages: 3, promptChars: 7000 }, PRICING)], PRICING);
  const base = p.request as unknown as VideoJobRequest;
  const res = base.resolution ?? VIDEO_CAPABILITIES.defaultResolution;
  const parts = [];
  ...
  let acc = 0;
  for (const [i, s] of plan.segments.entries()) {
    parts.push(estimateVideo({ resolution: res, outputSeconds: s.durationSec, promptChars: String(base.prompt).length, imageInputs: i === 0 ? (base.media ?? []).length : 0, videoInputSeconds: acc, task: i === 0 ? 'generate' : 'extend' }, PRICING));
    acc += s.durationSec;
  }
  parts.push(estimateInspection({ modelId: MODEL_REGISTRY.reasoning.id, durationSec: acc || plan.plannedSec, referenceImages: 3, promptChars: 7000 }, PRICING));
  return sumEstimates(parts, PRICING);
}
