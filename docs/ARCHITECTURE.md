...
- **Inspection** (`functions/src/workers/inspect.ts`) measures the real file (speech bounds, trailing room, clipping, loudness, scene cuts, black frames, end motion), transcribes it with word timestamps (`gemini-3.5-transcribe-preview`), aligns the transcript to the screenplay (`packages/shared/src/text-align.ts`: missing, altered, repeated, truncated words, early/late start, cutoff time), and has `gemini-3.8-flash` watch the video with the reference images for the scene review (speaker attribution, lip-sync, action beats, continuity, artefacts, first/last frames, scores) and the continuity director (characters, set, blocking, direction, text, props, temporal faults, edges, detected state, 15 category scores). The 32-section schema goes in the prompt (JSON mode): Gemini refuses it as a response schema (too complex to enforce), and enforcing it in parts made the scores collapse to flat, unexplained values. Every reply is checked against the schema before anything is scored; an incomplete or mistyped reply fails the inspection (retried) instead of being read with default values. The verdict (`evaluateQuality`) is deterministic: incomplete dialogue caps the score at 55, incomplete action at 60, critical faults at 40; a failing version can only be approved through the API with the waived issues recorded. Picture cuts are checked against the plan: joins between connected shots and the reaction-shot / reverse-angle cuts a continuation part was directed to make (up to two per part) are intended; any other cut is reported, and fails the scene when the reviewer also sees it.
...
- **Inspection and repair.** Frames are read with Cloud Vision (faces, objects, text — also mirrored), measured over time (flicker, freezes, held frames — a frame that barely changes while its neighbours move, from FFmpeg `signalstats` YDIF, so a calm shot is never called stutter — colour drift, subject tracks) and reviewed by the reasoning model against the plan; repairs escalate from the least destructive (colour match, screen composite, blocking frame, conversational edit, background replacement, trim, cutaway, section regeneration, stronger references, another approved take) and every repaired version is re-inspected.
